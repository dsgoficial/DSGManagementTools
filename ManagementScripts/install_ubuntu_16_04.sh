...
#preparing QGIS repository
	if grep -q "deb http://qgis.org/ubuntugis-ltr/ xenial main" /etc/apt/sources.list; then
		echo "repositório QGIS LTR já adicionado"
	else
		$SUDO echo "deb http://qgis.org/ubuntugis-ltr/ xenial main" | sudo tee -a /etc/apt/sources.list
	fi
...
#installing packages
	$SUDO apt-get install synaptic qgis saga python-saga otb-bin python-otb otb-bin-qt grass qgis-plugin-grass postgresql postgis slony1-2-bin postgresql-9.5-slony1-2 pgadmin3 apache2 libapache2-mod-python python-qt4-sql libqt4-sql-psql libqt4-sql-sqlite
